---
layout: course
title: LING 102 - Introduction to the Study of Language
description: Introduction to major topics and methods of analysis in linguistics, with a focus on producing written works.
year: 
term: Fall 2026, Spring 2026, Fall 2025
location: TBA
time: Monday Wednesday Friday, 9:30-10:20 am
course_id: 
schedule:
  - week: 1
    date: 
    topic: Syllabus and Roadmap
    description: Overview of classroom expectations and coursework.
    materials:
      - name: Syllabus
        url: /assets/pdf/example_pdf.pdf
      - name: Slides
        url: /assets/pdf/example_pdf.pdf
      - name: Beginning of Semester Survey
        url: /assets/pdf/example_pdf.pdf
      - name: Linguistic Autobiography
        url: /assets/pdf/example_pdf.pdf

  - week: 2
    date: 
    topic: 
    description: 
    materials:
      - name: Short Essay 1
        url: /assets/pdf/example_pdf.pdf

  - week: 3
    date: 
    topic: 
    description: 
    materials:
      - name: 
        url: 

  - week: 4
    date: 
    topic: 
    description: 
    materials:
      - name: Short Essay 2
        url: /assets/pdf/example_pdf.pdf

  - week: 5
    date: 
    topic: 
    description: 
    materials:
      - name: Review Materials
        url: /assets/pdf/example_pdf.pdf

  - week: 6
    date: 
    topic: 
    description: 
    materials:
      - name: Short Essay 3
        url: /assets/pdf/example_pdf.pdf

  - week: 7
    date:
    topic:
    description:
    materials:

  - week: 8
    date:
    topic:
    description:
    materials:
      - name: Language Report
        url:

  - week: 9
    date:
    topic:
    description:
    materials:

  - week: 10
    date:
    topic:
    description:
    materials:

  - week: 11
    date:
    topic:
    description:
    materials:

  - week: 12
    date:
    topic:
    description:
    materials:

  - week: 13
    date:
    topic:
    description:
    materials:

  - week: 14
    date:
    topic:
    description:
    materials:

  - week: 15
    date:
    topic:
    description:
    materials:

  - week: 16
    date:
    topic:
    description:
    materials:
---

## Course Overview

This course provides a comprehensive introduction to the major subfields and general topics in linguistics. Students will:

- 
- 
- 
- 


## Textbooks

- No textbook for the class

